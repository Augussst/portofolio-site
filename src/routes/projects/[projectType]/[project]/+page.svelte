			...
			<div class="w-full">
				...
				{#if meta.projectImages?.length > 0 && meta.url}
					<div class="flex w-full gap-2 px-4">
						<button
							type="button"
							class="flex-1 border py-1.5 px-3 rounded-t-xl text-sm flex items-center justify-center gap-1.5 transition-colors {activeMediaTab ===
							'images'
				...
				<!-- TAMPILAN GAMBAR -->
				{#if meta.projectImages?.length > 0 && (!meta.url || activeMediaTab === 'images')}
					...
					<!-- TAMPILAN IFRAME / EMBED (Diberi wrapper aspect-video yang persis sama) -->
				{:else if meta.url && (!meta.projectImages?.length || activeMediaTab === 'embed')}
					<div class="w-full aspect-video border rounded-xl overflow-hidden bg-black/5">
						<Iframe
				...
				{:else if meta.file}
					...
					<video class="w-full aspect-video border rounded-xl overflow-hidden" controls>
				...
				<!-- Tombol Aksi di Bawah Media -->
				{#if meta.sourceCode || meta.liveDemo || meta.url}
					<div class="flex flex-wrap gap-2 mt-3 justify-center md:justify-end">
						{#if meta.sourceCode}
							<a
						...
						{#if meta.liveDemo}
							...
							>
								...
								>
									...
									/>
									<path d="m11 1l3.3 3.3L8.6 10l1.4 1.4l5.7-5.7L19 9V1z" />
								</svg>
								Live Demo
							</a>
						{/if}
